...
def obter_ids_do_arquivo_local(estado):
    ...
    try:
        ...
        def is_id(c): return c.lower().strip() in ['identificador_unico', 'identificador único', 'id_unico']
        
        try: df = pd.read_csv(arquivo_recente, sep=sep, usecols=is_id, dtype=str, low_memory=True)
...
def salvar_lote(dados, caminho_csv):
    ...
    mapeamento = {
        'idUnico': 'id_unico_api', 'funcaoSocial': 'funcao_social',
        'metaGlobal': 'meta_global', 'dataInicialPrevista': 'data_inicial_prevista',
        'dataFinalPrevista': 'data_final_prevista', 'dataInicialEfetiva': 'data_inicial_efetiva',
        'dataFinalEfetiva': 'data_final_efetiva', 'dataCadastro': 'data_cadastro',
        'naturezaOutras': 'natureza_outras', 'descPlanoNacionalPoliticaVinculado': 'desc_plano_nacional_politica_vinculado',
        'qdtEmpregosGerados': 'qdt_empregos_gerados', 'descPopulacaoBeneficiada': 'desc_populacao_beneficiada',
        'populacaoBeneficiada': 'populacao_beneficiada', 'observacoesPertinentes': 'observacoes_pertinentes',
        'isModeladaPorBim': 'is_modelada_por_bim', 'dataSituacao': 'data_situacao', 'fontesDeRecurso': 'fontes_de_recurso'
    }
...
def process_turbo():
    ...
    for estado in estados:
        ...
        for i, pid in enumerate(ids_pendentes, 1):
            ...
            sys.stdout.write(f"\r   [{i}/{total}] Baixando: {clean_pid}   ")
            sys.stdout.flush()
